...
class GradeController extends Controller
{
     ...
     */
    public function gradeCreateAction(Request $request)
    {
      $session = new Session();
      ...
      if($session->get('user_id') && ($session->get('user_type') == 'admin')){
        ...
        if($form->isSubmitted()){
          ...
          return $this->redirect($this->generateUrl('grade_view', array('grade_system_id' => $grade_data['id'])));
        }
     ...
     */
    public function gradeEditAction(Request $request, $grade_id)
    {
      $data = [];
      $data['mode'] = 'edit';
    ...
    public function gradeDeleteAction(Request $request, $grade_id)
    {
      ...
      $grade = $this->getDoctrine()
                          ->getRepository('AppBundle:Grade')
                          ->findOneById($grade_id);
      $grade_data['id'] = $grade->getId();
      $grade_data['created_by'] = $grade->getCreatedBy();

      if($session->get('user_name') && ($session->get('user_type') == 'admin') && ($session->get('user_id') == $grade_data['created_by'])){
        ...
        $data['message'] = 'You Are Not Qualified to Delete This Grade.';
        return $this->render('accessDenied.html.twig', $data);
      }
    }
