...
class StudentProfileController extends Controller
{
    ...
    /**
     * @Route("/student_profile/edit/{student_id}", name="student_profile_edit")
     */
    public function studentEditAction(Request $request, $student_id)
    {
      ...
      if(($session->get('user_name') == $student_data['user_name']) && ($session->get('user_type') == 'student')){
        ...
        if($form->isSubmitted()){
          ...
          return $this->redirect($this->generateUrl('account_settings', array('user_id' => $session->get('user_id'))));
        }
        return $this->render('student/profile_form.html.twig', $data);
      } else {
    ...
    /**
     * @Route("/student/profile_view/{student_id}", name="student_profile_view")
     */
    public function studentViewOneAction(Request $request, $student_id)
    {
      $session = new Session();

      if(($session->get('user_id') == $student_id) && ($session->get('user_type') == 'student')){
        $data = [];
        $data['students'] = [];
        ...
        return $this->render('student/profile_view.html.twig', $data);
      } else {
